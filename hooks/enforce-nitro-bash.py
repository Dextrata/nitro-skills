...
of rerun, grep instead of ripgrep). This hook denies those commands and names
the skill to use instead. Append `#nitro-skip` to a command to let it through.
...
Rules, first match wins:
  ...
  3. cat/type/Get-Content/sed -n/head/tail dumping more than LINES lines of an
     existing file, not piped onward -> hashpatch outline/grep/view, or probe.
  4. In-place edits of an existing file (sed -i, perl -i, > or >> redirection,
     tee, Set-Content/Out-File/Add-Content, python -/-c or node -e that writes
     a file) -> hashpatch apply. Redirecting into a new file is allowed.
  5. A test/build/lint command not wrapped in rerun (rr.py) -> rerun.
"""
import json
import os
import re
...
def main():
    ...
    if EDIT.search(cmd):
        return deny("In-place shell edits (sed -i, perl -i, tee, Set-Content, Out-File) are blocked. Use the hashpatch "
                    "skill: `grep`/`view` to get anchors, then `apply` with hunks.")
